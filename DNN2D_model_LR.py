...
if __name__ == "__main__":
    logging.basicConfig(filename='./DNN2D_model_LR.log', level=logging.INFO)
    ...
    BAR_1L_Y = np.ones(BAR_1L.shape[0], dtype=np.float16)*(-10)

    BAR_2L = Normalization(pd.DataFrame(BAR_2L.data))
    BAR_2L_Y = np.ones(BAR_2L.shape[0], dtype=np.float16)*(-20)

    BAR_1R = Normalization(pd.DataFrame(BAR_1R.data))
    BAR_1R_Y = np.ones(BAR_1R.shape[0], dtype=np.float16)*10

    BAR_2R = Normalization(pd.DataFrame(BAR_2R.data))
    BAR_2R_Y = np.ones(BAR_2R.shape[0], dtype=np.float16)*20
    ...
    # model = tf.keras.models.load_model('DNN_2D_LR.h5')
    ...
    csv_logger = CSVLogger('./DNN2D_model_LR_epochs.log', append=True, separator=';')
