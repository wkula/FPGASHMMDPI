...
def RNN_model(shape=(9,)):
    din = tf.keras.Input(shape=shape)
    x = tf.keras.layers.LSTM(64, return_sequences=True)(din)
    x = tf.keras.layers.Dropout(0.2)(x)
...
def SequencingData(x_data: pd.DataFrame, y_data: pd.DataFrame, seq_len: int = 10, name='') -> tuple:
    ...
    step = 1 if seq_len < 300 else int(seq_len/200)
...
if __name__ == "__main__":
    logging.basicConfig(filename='./DNN2D_model_just_current.log', level=logging.INFO)
    ...
    BAR_C_Y = np.ones(BAR_C.shape[0], dtype=np.float16)
    ...
    print("Spliting sets")
    X, X_val, y, y_val = train_test_split(X, y, test_size=0.1, random_state=256)
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=256)

    print("Creating model")
    model = RNN_model(shape=(X_train.shape[1], X_train.shape[2]))
    ...
    csv_logger = CSVLogger('./DNN2D_model_just_current.log', append=True, separator=';')
